""" Note on how to access the results obtained by running this script:
    ...
    Check the function "leaveOneMeerkatOutResults" for a comprehensive list of
    all the sub-fields of results[i][1] (the aggregated results), results[i][2] 
    (the mean results), and results[i][3] (the meerkat-wise results)
...
excludeNonRestingMeerkats = 1 # if this is one, the "non-resting" meerkats will be excluded from lomoCV. If this is 0, lomoCV will be perfomed over all meerkats
...
for name,model in models:
    ...
    aggregatedResults, meanResults, meerkatWiseResults = leaveOneMeerkatOutResults(model,X,Y,N,balanceMethod=balanceMethodStr)
    
    resultsA.append((name, aggregatedResults, meanResults, meerkatWiseResults))
...
print("The meerkat-wise results for the statistical features (features A) have been computed")
...
for name,model in models:
    ...
    resultsB.append((name, aggregatedResults, meanResults, meerkatWiseResults))
    # this row of "results" contains the name of the model run in this iteration, its aggregated (across all folds) results, and its fold-wise results
    
    msg = "%s finished running" % (name)
    print(msg)
    
print("The meerkat-wise results for the human activity features (features B) have been computed")
...
print("The meerkat-wise results for the combined feature-set (features ABtogether) have been computed")
...
for name,model in models:
    ...
    resultsC.append((name, aggregatedResults, meanResults, meerkatWiseResults))
    # this row of "results" contains the name of the model run in this iteration, its aggregated (across all folds) results, and its fold-wise results
    
    msg = "%s finished running" % (name)
    print(msg)
    
print("The meerkat-wise results for the biomechanical features (features C2) have been computed")


#%% Save all results

# Saving the leave-one-meerkat-outCVresults
with open('results_ML_lomoCV_allFeatures_restingMeerkatOnly.pickle', 'wb') as f:
    pickle.dump([resultsA, resultsB, resultsABtogether, resultsC], f)
    
#==============================================================================
# # Loading the results once they've been computed and saved
# with open('results_ML_lomoCV_allFeatures_restingMeerkatOnly.pickle','rb') as f:
...
savetxt('table[01b2]_ML_lomo_allFeatures_agg.csv', resultsMatrix, delimiter=',', fmt='%.1f')
...
filename = 'table[01b2]_ML_lomo_allFeatures_meanstd.csv'
