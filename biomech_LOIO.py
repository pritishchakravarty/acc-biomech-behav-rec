...
The cross-validation method used is leave-one-meerkat-out cross-validation.
...
from crossValidationFunctions import leaveOneMeerkatOutResults_biomechanicalModel
    ...
    for i in range(len(models)):        # for each static VS dynamic model
        for j in range(len(models)):            # for each vig VS resting model
            for k in range(len(models)):                # for each foraging VS running model
                ...
                # finding the results
                aggregated, meanFold, foldWise = leaveOneMeerkatOutResults_biomechanicalModel(threeModels,F,X,Y,N,Y_SD,balanceMethod=balanceMethodStr)
...
with open('results_biomechanicalModels_lomo_restingMeerkats.pickle', 'wb') as f:
    pickle.dump([results, featureIdx, featuresC2_names], f)

#==============================================================================
# # Loading the saved results
# with open('results_biomechanicalModels_lomo_restingMeerkats.pickle','rb') as f:
...
for featureSetIdx in range(len(featureIdx)):
    ...
    ff = featureSetIdx + 1
    filename = 'table[03b2]_FS' + ('%ia_agg.csv' % ff)
...
for featureSetIdx in range(len(featureIdx)):
    ...
    ff = featureSetIdx + 1
    filename = 'table[03b2]_FS' + ('%ib_meanstd.csv' % ff)
